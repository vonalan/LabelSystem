...
class videoLabel:
    def __init__(self, videoDir, imageDir, labelName):
        ...
        # self.storeframe = []    # 用于按键A倒退、按键D前进的相关设置
        self.storename = []    # 用于按键A倒退、按键D前进的相关设置
        self.storerects = []
        self.prefix_template = 'd:/labelling/template_prefix.xml'    # 这两个是输出xml的相关设置
        self.object_template = 'd:/labelling/template_object.xml'
    ...
    def draw_static(self, name, frame, shape, key, rects):
        ...
        for k, v in self.classes.items():
            ...
            cv2.putText(frame, chr(v), coord, self.font, self.fontsize, (0, 0, 255), 2, cv2.LINE_AA)
    ...
    def update_storerects(self, rects0, rects1, idx_f):
        ...
        for i in range(self.length):
            ...
            self.storerects.append(currect)
...
#                cv2.rectangle(self.frame, tuple(points[0]), tuple(points[1]), (0,255,0), thickness=self.linethick)
        ...
        for k, v in self.classes.items():
            box = self.rects[k]
            coord = (int((box[0][0] + box[1][0]) / 2), box[1][1])
            cv2.putText(self.frame, chr(v), coord, self.font, self.fontsize, (0, 0, 255), 2, cv2.LINE_AA)
    ...
    def labelling(self):
        ...
        while True:
            ...
            if key == 100:
                # 'd', 进入下一张图片
                self.draw_static(name, self.frame, self.shape, key, self.rects)
                print('D: %s, %s, write_name,%s' % (idx, idx_f, name))
                if idx < idx_f:
                    idx += 1
                    name, self.frame, self.bufframe, self.shape = self.update(idx)
                    print('op_name,%s' % name)
            ...
            if key == 97:
                ...
                if idx > (idx_f-30):
                    ...
                    if self.dr == 1:
                        ...
                        for i in range(self.length):
                            name, self.frame, self.bufframe, self.shape = self.update(idx_f-i)
                    ...
                    # print('error: idx_f,%s,  len:%s' % (idx_f, len(self.storerects)))

                    self.writeLog(str(name) + ' , ' + chr(key))
                    print('A: idx,%s, op_name,%s' % (idx, name))
...
if __name__ == '__main__':
    ...
    labelName = r'D:\Users\Administrator\Desktop\HGR\VideoLabel-DF\labels.txt'
    vl = videoLabel(videoDir, imageDir, labelName)
    vl.labelling()
