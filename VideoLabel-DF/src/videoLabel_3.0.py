...
#        elif event == cv2.EVENT_RBUTTONUP:
    ...
    def realTimeLabel(self, key=None):
        '''
        ...
        '''Each big box be with 4 small boxes'''
        for i, pts in enumerate(self.rects):
            cv2.rectangle(self.frame, tuple(pts[0]), tuple(pts[1]), (0, 255, 0), thickness=self.linethick)
            points = [pts[0], pts[1], [pts[1][0], pts[0][1]], [pts[0][0], pts[1][1]]]
            # point = [[x1,y1],[x2,y2],[x2,y1],[x1,y2]
        ...
        for k, v in self.classes.items():
            box = self.rects[k]
            coord = (int((box[0][0] + box[1][0]) / 2), box[1][1])
            # cv2.putText(self.frame, chr(v), coord, self.font, self.fontsize, (0, 255, 0), 2, cv2.LINE_AA)

            ###
            # labelColorsList = [(0, 0, 0), (0, 255, 0), (255, 0, 0), (0, 0, 255), (255, 255, 66),
            #                    (0, 122, 122), (122, 0, 122), (122, 0, 122), (122, 0, 122), (122, 0, 122)]
            cv2.putText(self.frame, chr(v), coord, self.font, self.fontsize, self.colorList[v-48-1], 2,
                        cv2.LINE_AA)
