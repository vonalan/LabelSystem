...
#        elif event == cv2.EVENT_RBUTTONUP:
    ...
    def labelling(self, batch):
        ...
        numFrame = len(self.frameList)
        self.storename = ['%s_%d.png'%(self.video, batch*300+(i+1)) for i in range(numFrame)]
        ...
        while True:
            ...
            if key in list(map(ord, self.labels)):
                ...
                if self.chooseRect >= 0:
                    ...
                    '''两种选择：
                    1. 按下F之后标签不能再改变；
                    2. 标签改变之后所对应的rects也要实时改变，但是不写回内存。
                    '''
                    if self.storerects:
                        self.storerects[cur_idx][self.chooseRect][0] = self.classes[self.chooseRect] ### 废话
            ...
            if idx_itv[1] == numFrame and self.dr == 0 and self.fc == True:
                break

        self.flush_storerects(key)
        cv2.destroyAllWindows()
        return None

    def iter_mini_batches(self, videoPath):
        frameList = []
    ...
    def labelThemAll(self):
        self.videoList = os.listdir(self.videoDir)
        for video in self.videoList:
            self.video = video
            self.frameList = []

            videoPath = os.path.join(self.videoDir, video)
            for batch, frameList in enumerate(self.iter_mini_batches(videoPath)):
                self.frameList = frameList
                self.labelling(batch)
                '''bug bug bug'''
                break
