...
#            print box[2],box[4],  box[1], box[3]
    ...
    #
    #     tmpshape = [1080, 1920]
    #     tmpshape1 = [int(i * 0.9) for i in tmpshape]
    ...
    def resize(self, frame=None, scale=None):
        ...
        r0 = objshape[0] / float(orishape[0])
        r1 = objshape[1] / float(orishape[1])
        scale = max(r0, r1) # !!! scale = min(r0, r1)
    ...
    def change(self, srcimgdir, srcxmldir, logname, dstimgdir, dstxmldir, objectives=[]):
        ...
        for nameIdx, _ in enumerate(nameList):
            ...
            if flag:
                ...
                sizes = self.xmlParser.parsing_sizes_dev(xmlname) # [960, 544]
                self.sizes = sizes
                ...
                switch3 = not (switch1 or switch2) # 是否需要重新标注

                if switch1: count[0]+=1 
                if switch2: count[1]+=1 
                if switch3: count[2]+=1 
...
if __name__ == '__main__':
    srcimgdir = r'F:\Users\Kingdom\Desktop\VideoLabel-DF\outputs\imgs' # 图片地址
    srcxmldir = r'F:\Users\Kingdom\Desktop\VideoLabel-DF\outputs\xmls' # 原来的xmls地址
    dstimgdir = r'F:\Users\Kingdom\Desktop\VideoLabel-DF\outputs\dbgs\imgs'
    dstxmldir = r'F:\Users\Kingdom\Desktop\VideoLabel-DF\outputs\dbgs\xmls' # 新的xmls输出地址
    prefix_template = 'template_prefix.xml'
    object_template = 'template_object.xml'
    logname = 'visual.log'

    ls = changeSizes(srcimgdir, srcxmldir, prefix_template, object_template, logname)
